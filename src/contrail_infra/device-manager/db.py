...
class PhysicalRouterDM(DBBaseDM):
    ...
    def update(self, obj=None):
        ...
        self.mx_type = obj.get('physical_router_mx_type', '')
        ...
        self.update_single_ref('routing_protocols', obj)
        self.update_multiple_refs('virtual_network', obj)
        self.physical_interfaces = set([pi['uuid'] for pi in
                                        obj.get('physical_interfaces', [])])
        self.fabric_interfaces   = set([fi['uuid'] for fi in
                                        obj.get('fabric_interfaces', [])])
    ...
    def delete(cls, uuid):
        ...
        if obj.is_vnc_managed() and obj.is_conf_sent():
            ...
            obj.config_manager.delete_mx_config()
        obj._cassandra.delete_pr(uuid)
        obj.uve_send(True)
        obj.update_single_ref('bgp_router', {})
        obj.update_single_ref('routing_protocols', {})
	...
	#import pdb; pdb.set_trace() # comment this for now
        vn_dict = {}
    ...
    def delete_config(self):
        if not self.is_vnc_managed() and self.is_conf_sent():
            ...
            self.config_manager.delete_mx_config()
    ...
    #def add_phy_intf_config(self, pi_pi):
        #li_set = pi_pi.logical_interfaces
        #for li_uuid in li_set:
            #li = LogicalInterfaceDM.get(li_uuid)
            #if li is None:
                #continue
            #self.config_manager.add_mx_phy_logical_interface(pi.physical_interface_type, 
			    #pi.physical_interface_encap_type, li.name, li.li_type, li.li_encap_type,
			    #li.vlan_tag, li.li_family, li.li_ipaddress)
    # end
    ...
    def config_pnf_logical_interface(self):
        ...
        for pi_uuid in self.physical_interfaces:
	        ...
	        #import pdb; pdb.set_trace() # comment this for now
                #self.add_phy_intf_config(pi_pi)
    ...
    def config_mx_phy_logical_interface(self):
        status = False
        for pi_uuid in self.physical_interfaces:
            pi = PhysicalInterfaceDM.get(pi_uuid)
            if pi is None:
                continue
            if pi.physical_interface_prop is not None:
                status = True
                self.config_manager.add_mx_phy_intf(pi.name, 
                                                    pi.physical_interface_prop,
                                                    pi.physical_interface_val)
            if pi.name.startswith("ps"):
                self.config_manager.add_mx_ps_intf()
            li_set = pi.logical_interfaces
            for li_uuid in li_set:
                li = LogicalInterfaceDM.get(li_uuid)
                if li is None:
			    ...
			    pi.physical_interface_encap_type, li.name, li.li_type, li.li_encap_type,
			    li.vlan_tag, li.li_family, li.li_ipaddress)
                if li.name.startswith("lo0"):
                    self.config_manager.add_mx_routing_options(li.li_ipaddress)
        for pi_uuid in self.fabric_interfaces:
            fi = FabricInterfaceDM.get(pi_uuid)
            if fi is None:
                continue
            li_set = fi.logical_interfaces
            for li_uuid in li_set:
                li = LogicalInterfaceDM.get(li_uuid)
                if li is None:
                    continue
                status = True
                self.config_manager.add_mx_phy_logical_interface(0,'0',
				li.name, li.li_type, li.li_encap_type,
				li.vlan_tag, li.li_family, li.li_ipaddress)
        return status
    # end config_mx_phy_logical_interface

    def config_mx_routing_protocols(self, routing):
	#import pdb; pdb.set_trace()
        if routing.mpls_protocol is not None:
            self.config_manager.add_mpls_protocol_config_xml(routing.mpls_protocol.get('interface_name', []))
        if routing.ospf_protocol is not None:
            self.config_manager.add_ospf_protocol_config_xml(routing.ospf_protocol.get('interface_name', []), 
			    routing.ospf_protocol.get('area'))
        if routing.ldp_protocol is not None:
            self.config_manager.add_ldp_protocol_config_xml(routing.ldp_protocol.get('interface_name', []))
        if routing.l2ckt_protocol is not None:
            self.config_manager.add_l2ckt_protocol_config_xml(routing.l2ckt_protocol.get('virtual_ckt_id'), 
			    routing.l2ckt_protocol.get('identifier'), 
			    routing.l2ckt_protocol.get('interface_name'))
    # end config_mx_routing_protocols
    ...
    def push_config(self):
        ...
        self.config_manager.reset_mx_config()
        #set the state here so based on the type the config is sent.
	self.config_manager.mx_type = self.mx_type;
        ...
        # mx config
	#import pdb; pdb.set_trace()
        status = self.config_mx_phy_logical_interface()
        routing_protocols = RoutingProtocolsDM.get(self.routing_protocols)
        if routing_protocols and status:
            self.config_mx_routing_protocols(routing_protocols)
        config_size = self.config_manager.send_mx_config()
...
class PhysicalInterfaceDM(DBBaseDM):
    ...
    def __init__(self, uuid, obj_dict=None):
        ...
        self.physical_interface_type = None
        self.physical_interface_prop = None
        self.physical_interface_val = None
        self.physical_interface_encap_type = None
        self.update(obj_dict)
    ...
    def update(self, obj=None):
        ...
        if obj.get('physical_interface_type', None):
            self.physical_interface_type = obj.get("physical_interface_type", 0)
            self.physical_interface_prop = self.physical_interface_type.get(
                                               'interfacetype', None)
            self.physical_interface_val = self.physical_interface_type.get(
                                               'value', None)
        self.physical_interface_encap_type = obj.get("physical_interface_encap_type", 0)
    # end update
...
class FabricInterfaceDM(DBBaseDM):
    _dict = {}
    obj_type = 'fabric_interface'

    def __init__(self, uuid, obj_dict=None):
        #import pdb; pdb.set_trace()
        self.uuid = uuid
        self.fabric_interfaces = set()
        self.fabric_physical_interfaces = None
        self.update(obj_dict)
        pr = PhysicalRouterDM.get(self.physical_router)
        if pr:
            pr.fabric_interfaces.add(self.uuid)
	#import pdb; pdb.set_trace()
    # end __init__
    ...
    def delete(cls, uuid):
        ...
        pr = PhysicalRouterDM.get(obj.physical_router)
        if pr:
            pr.fabric_interfaces.discard(obj.uuid)
        del cls._dict[uuid]
    # end delete
# end FabricInterfaceDM
...
class LogicalInterfaceDM(DBBaseDM):
    ...
    def __init__(self, uuid, obj_dict=None):
        ...
        self.vlan_tag = 0
        self.li_type = None
        self.li_encap_type = None
        self.li_family = None
        self.li_ipaddress = None
        self.update(obj_dict)
        ...
        if self.physical_interface:
            parent = FabricInterfaceDM.get(self.physical_interface)
        if parent:
            parent.logical_interfaces.add(self.uuid)
	#import pdb; pdb.set_trace()
    # end __init__
    ...
    def update(self, obj=None):
        ...
        self.li_encap_type = obj.get("logical_interface_encap_type", 0)
        self.li_family = obj.get("logical_interface_family", 0)
        self.li_ipaddress = obj.get("logical_interface_ipaddress", 0)
    ...
    def delete(cls, uuid):
        ...
        if obj.physical_interface:
            parent = FabricInterfaceDM.get(obj.physical_interface)
        if parent:
            parent.logical_interfaces.discard(obj.uuid)
...
class VirtualMachineInterfaceDM(DBBaseDM):
    ...
    def is_device_owner_bms(self):
	    ...
	    if self.device_owner:
	        if self.device_owner.lower() == 'physicalrouter':
                    return True
        return False
    # end
...
class RoutingProtocolsDM(DBBaseDM):
    _dict = {}
    obj_type = 'routing_protocols'

    def __init__(self, uuid, obj_dict=None):
        #import pdb; pdb.set_trace()
        self.uuid = uuid
        self.physical_router = None
        self.ldp_protocol = None
        self.ospf_protocol = None
        self.mpls_protocol = None
        self.l2ckt_protocol = None
        self.update(obj_dict)
    # end __init__
    ...
    def update(self, obj=None):
        ...
        if obj is None:
            obj = self.read_obj(self.uuid)
        self.name = obj['fq_name'][-1]
        self.ldp_protocol = obj['routing_protocol_ldp']
        self.ospf_protocol = obj['routing_protocol_ospf']
        self.mpls_protocol = obj['routing_protocol_mpls']
        self.l2ckt_protocol = obj['routing_protocol_l2ckt']
        self.update_single_ref('physical_router', obj)
