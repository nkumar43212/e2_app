...
class PhysicalRouterConfig(object):
    ...
    def send_mx_netconf(self, new_config, default_operation="merge",
		        operation="replace"):
        #import pdb; pdb.set_trace()
        self.push_config_state = PushConfigState.PUSH_STATE_INIT
        start_time = None
    ...
    def set_mx_config(self, params1, params2, params3, params4, params5):
        self.mx_phy_intf_config = params1
        self.mx_fab_intf_config = params2
        self.mx_routing_options_config = params3
        self.mx_routing_protocols_config = params4
        self.mx_chassis_config = params5
    # end set_mx_config

    def reset_mx_config(self):
        self.mx_phy_intf_config = None
        self.mx_fab_intf_config = None
        self.mx_routing_options_config = None
        self.mx_routing_protocols_config = None
        self.mx_chassis_config = None
        self.mx_type = None
    # end reset_mx_config

    def delete_mx_config(self):
        self.reset_mx_config()
        self.send_mx_netconf([], default_operation="none", operation="delete")
    # end delete_mx_config

    def _get_interface_config_xml(self):
        interface_config = etree.Element("interface")
        etree.SubElement(interface_config, "name").text = "ge-0/0/0"
    ...
    def _get_interface_fabric0_config_xml(self):
        ...
        etree.SubElement(address, "name").text = "200.0.0.1/24"
        family = etree.SubElement(unit, "family")
        etree.SubElement(family, "mpls")
        return fabric_config
    # end _get_interface_fabric0_config_xml

    def _get_interface_fabric1_config_xml(self):
        fabric_config = etree.Element("interface")
        ps_name = etree.SubElement(fabric_config, "name").text = "ps0"
	anp = etree.SubElement(fabric_config, "anchor-point")
	etree.SubElement(anp, "interface-name").text = "lt-0/0/0"
        unit = etree.SubElement(fabric_config, "unit")
        etree.SubElement(unit, "name").text = "0"
        etree.SubElement(unit, "encapsulation").text = "ethernet-ccc"
	etree.SubElement(fabric_config, "flexible-vlan-tagging")
        unit = etree.SubElement(fabric_config, "unit")
        etree.SubElement(unit, "name").text = "1"
        etree.SubElement(unit, "vlan-id").text = "100"
        family = etree.SubElement(unit, "family")
        inet_family = etree.SubElement(family, "inet")
        address = etree.SubElement(inet_family, "address")
        etree.SubElement(address, "name").text = "110.0.0.2/24"
        unit = etree.SubElement(fabric_config, "unit")
        etree.SubElement(unit, "name").text = "2"
        etree.SubElement(unit, "vlan-id").text = "101"
        family = etree.SubElement(unit, "family")
        inet_family = etree.SubElement(family, "inet")
        address = etree.SubElement(inet_family, "address")
        etree.SubElement(address, "name").text = "111.0.0.2/24"
        return fabric_config
    # end _get_interface_fabric1_config_xml

    def _get_interface_fabric_child_config_xml(self, phy_name, phy_ch_name):
	#import pdb; pdb.set_trace()
        ...
        if phy_ch_name.startswith("fi"):
           phy_ch_name_r = phy_ch_name.replace("fi","ae")
        etree.SubElement(ad, "bundle").text = phy_ch_name_r
        return fabric_child
    # end _get_interface_fabric_child_config_xml

    def _get_interface_lo0_config_xml(self):
        ...
        inet_family = etree.SubElement(family, "inet")
        address = etree.SubElement(inet_family, "address")
        etree.SubElement(address, "name").text = "1.1.1.1/24"
        return lo0_config
    # end _get_interface_lo0_config_xml

    def _get_chassis_config_access_xml(self):
        chassis_config = etree.Element("chassis")
        ad = etree.SubElement(chassis_config, "aggregated-devices")
        eth = etree.SubElement(ad, "ethernet")
        etree.SubElement(eth, "device-count").text = "2"
        etree.SubElement(chassis_config, "network-services").text = "enhanced-ip"
        return chassis_config
    # end _get_chassis_config_access_xml

    def _get_chassis_config_service_xml(self):
        chassis_config = etree.Element("chassis")
        ad = etree.SubElement(chassis_config, "aggregated-devices")
        eth = etree.SubElement(ad, "ethernet")
        etree.SubElement(eth, "device-count").text = "2"
        ...
        return chassis_config
    # end _get_chassis_config_service_xml

    def _get_routing_options_config_xml(self, lo0_ip):
        routing_options = etree.Element("routing-options")
        lo0_ipaddr = lo0_ip.split('/', 1)
        #etree.SubElement(routing_options, "router-id").text = "1.1.1.1"
        etree.SubElement(routing_options, "router-id").text = lo0_ipaddr[0]
        etree.SubElement(routing_options, "route-distinguisher-id").text = lo0_ipaddr[0]
        return routing_options
    # end _get_routing_options_config_xml

    def _get_protocols_config_xml(self):
        protocol_config = etree.Element("protocols")
        mpls_cfg = etree.Element("mpls")
        mpls_intf = etree.SubElement(mpls_cfg, "interface")
        etree.SubElement(mpls_intf, "name").text = "ae0.0"
        ...
        etree.SubElement(ospf_intf, "name").text = "ae0.0"
        ospf_intf = etree.SubElement(ospf_area, "interface")
        etree.SubElement(ospf_intf, "name").text = "lo0.0"
        ldp_cfg = etree.Element("ldp")
        ldp_intf = etree.SubElement(ldp_cfg, "interface")
        etree.SubElement(ldp_intf, "name").text = "ae0.0"
        ldp_intf = etree.SubElement(ldp_cfg, "interface")
        etree.SubElement(ldp_intf, "name").text = "lo0.0"
        l2ckt_cfg = etree.Element("l2circuit")
        l2ckt = etree.SubElement(l2ckt_cfg, "neighbor")
        etree.SubElement(l2ckt, "name").text = "2.2.2.2"
        l2ckt_intf = etree.SubElement(l2ckt, "interface")
        etree.SubElement(l2ckt_intf, "name").text = "ge-0/0/0.0"
        etree.SubElement(l2ckt_intf, "virtual-circuit-id").text = "1"
        protocol_config.append(mpls_cfg)
        protocol_config.append(ospf_cfg)
        protocol_config.append(ldp_cfg)
        protocol_config.append(l2ckt_cfg)
        return protocol_config
    # end _get_protocols_config_xml

    def _get_interface_ps_config_xml(self):
        ps_config = etree.Element("interface")
        etree.SubElement(ps_config, "name").text = "ps0"
    ...
    def send_mx_config(self):
        #bsingarayan; debugging
        #import pdb; pdb.set_trace()
        if self.mx_type == 'access':
            self.mx_chassis_config = self._get_chassis_config_access_xml()
        elif self.mx_type == 'service':
            self.mx_chassis_config = self._get_chassis_config_service_xml()
        else:
            self.mx_chassis_config = self._get_chassis_config_xml()
        mx_chassis_config = self.mx_chassis_config
        config_list = [mx_chassis_config]
        if self.mx_phy_intf_config is not None:
            config_list.append(self.mx_phy_intf_config)
        if self.mx_fab_intf_config is not None:
            config_list.append(self.mx_fab_intf_config)
        if self.mx_routing_protocols_config is not None:
            config_list.append(self.mx_routing_protocols_config)
        if self.mx_routing_options_config is not None:
            config_list.append(self.mx_routing_options_config)
        #no element exists, so return.
        if len(config_list) == 1:
           return 0
        return self.send_mx_netconf(config_list)
    # end send_mx_config

    def add_mx_routing_options(self, lo0_ip):
        if self.mx_routing_options_config is None:
	    self.mx_routing_options_config = self._get_routing_options_config_xml(str(lo0_ip))
    # end add_mx_routing_options

    def add_mx_phy_intf(self, phy_name, phy_options, phy_ch_name):
        phy_config = self._get_interface_fabric_child_config_xml(phy_name, phy_ch_name)
        if self.mx_phy_intf_config is not None:
            self.mx_phy_intf_config.append(phy_config)
        else:
            intf_cfg = etree.Element("interfaces")
            intf_cfg.append(phy_config)
            self.mx_phy_intf_config = intf_cfg
    # end add_mx_phy_intf
 
    def add_mx_ps_intf(self):
        phy_config = self._get_interface_ps_config_xml()
        if self.mx_phy_intf_config is not None:
            self.mx_phy_intf_config.append(phy_config)
        else:
            intf_cfg = etree.Element("interfaces")
    ...
    def add_mx_phy_logical_interface(self, phy_type, phy_encap, li_name, li_type, li_encap, li_vlan_tag, li_family, li_ipaddress):
        fi_intf = 0
        insert_beg = 0
        li_config = etree.Element("interface")
        if li_name.startswith("fi"):
           li_name = li_name.replace("fi","ae")
           fi_intf = 1
        intf = li_name.split('.', 1)
        if li_name.startswith("ps"):
           if intf[1] == '0' or intf[1] == 0:
                #import pdb; pdb.set_trace()
                insert_beg = 1
        etree.SubElement(li_config, "name").text = intf[0]
        if li_vlan_tag != '0' and li_vlan_tag != 0:
            etree.SubElement(li_config, "flexible-vlan-tagging")
        if phy_encap != '0' and phy_encap != 0:
            etree.SubElement(li_config, "encapsulation").text = str(phy_encap)
        unit = etree.SubElement(li_config, "unit")
        etree.SubElement(unit, "name").text = intf[1]
        if li_vlan_tag != '0' and li_vlan_tag != 0:
            etree.SubElement(unit, "vlan-id").text = str(li_vlan_tag)
        if li_encap is not 0:
            etree.SubElement(unit, "encapsulation").text = str(li_encap)
        if li_family is not 0:
            family = etree.SubElement(unit, "family")
            li_family_str = str(li_family)
            fam_addr = etree.SubElement(family, li_family_str)
        if li_ipaddress is not 0 and li_family is not 0:
            addr = etree.SubElement(fam_addr, "address")
            etree.SubElement(addr, "name").text = li_ipaddress
        if fi_intf == 1:
            family = etree.SubElement(unit, "family")
            etree.SubElement(family, "mpls")
        if self.mx_phy_intf_config is not None:
            #insert ps0.0 at the beginning
            if insert_beg == 1:
                self.mx_phy_intf_config.insert(0, li_config)
            else:
                self.mx_phy_intf_config.append(li_config)
        else:
            intf_cfg = etree.Element("interfaces")
            intf_cfg.append(li_config)
            self.mx_phy_intf_config = intf_cfg
    # end add_mx_phy_logical_interface

    def add_mpls_protocol_config_xml(self, interfaces):
        mpls_cfg = etree.Element("mpls")
        for interface in interfaces:
            mpls_intf = etree.SubElement(mpls_cfg, "interface")
            #etree.SubElement(mpls_intf, "name").text = "ae0.0"
            if interface.startswith("fi"):
               interface = interface.replace("fi","ae")
            etree.SubElement(mpls_intf, "name").text = interface
        if self.mx_routing_protocols_config is not None:
            self.mx_routing_protocols_config.append(mpls_cfg)
        else:
            routing_cfg = etree.Element("protocols")
            routing_cfg.append(mpls_cfg)
            self.mx_routing_protocols_config = routing_cfg
    ...
    def add_ospf_protocol_config_xml(self, interfaces, area):
        ...
        #etree.SubElement(ospf_intf, "name").text = "lo0.0"
        if self.mx_routing_protocols_config is not None:
            self.mx_routing_protocols_config.append(ospf_cfg)
        else:
            routing_cfg = etree.Element("protocols")
    ...
    def add_ldp_protocol_config_xml(self, interfaces):
        ...
        #etree.SubElement(ldp_intf, "name").text = "lo0.0"
        if self.mx_routing_protocols_config is not None:
            self.mx_routing_protocols_config.append(ldp_cfg)
        else:
            routing_cfg = etree.Element("protocols")
    ...
    def add_l2ckt_protocol_config_xml(self, ckt_id, identifier, intf):
        l2ckt_cfg = etree.Element("l2circuit")
        l2ckt = etree.SubElement(l2ckt_cfg, "neighbor")
        #etree.SubElement(l2ckt, "name").text = "2.2.2.2"
        etree.SubElement(l2ckt, "name").text = identifier
        l2ckt_intf = etree.SubElement(l2ckt, "interface")
        #etree.SubElement(l2ckt_intf, "name").text = "ge-0/0/0.0"
        etree.SubElement(l2ckt_intf, "name").text = intf
        #etree.SubElement(l2ckt_intf, "virtual-circuit-id").text = "1"
        etree.SubElement(l2ckt_intf, "virtual-circuit-id").text = str(ckt_id)
        if self.mx_routing_protocols_config is not None:
            self.mx_routing_protocols_config.append(l2ckt_cfg)
        else:
            routing_cfg = etree.Element("protocols")
...
class JunosInterface(object):
    ...
    def __repr__(self):
        return "{name}({args})".format(name=self.__class__.__name__, args=', '.join("{k}={v!r}".format(k=k, v=v) for k,v in vars(self).items()))
